...
export class ClaimsService {
  ...
  async rajalVsRanapAnalysis(import_job_id: string) {
    ...
    const descriptionName =
      'DATA PASIEN MASUK "RAJAL DI HARI YANG SAMA", DAN "RAJAL BESERTA RANAP DI HARI YANG SAMA".';
  ...
  async getAnalyzedClaim(
    ...
    const baseQuery = this.claimResultsRepo
      .createQueryBuilder('claim_results')
      .innerJoin(Claim, 'claims', 'claims.id = claim_results.claim_id')
      .where('claim_results.import_job_id = :import_job_id', { import_job_id });

    if (normalizedGroupResults) {
      baseQuery.andWhere('claim_results.group_result = :group_results', {
    ...
    const [data, total] = await Promise.all([
      baseQuery
        .clone()
        .select([
          'claim_results.import_job_id AS import_job_id',
          'claim_results.claim_id AS claim_id',
          ...
          'claim_results.group_result AS group_result',
          'claim_results.sub_group_result AS sub_group_result',
          'claim_results.description AS description',
        ])
